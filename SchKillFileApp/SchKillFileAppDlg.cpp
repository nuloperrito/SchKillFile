...
BEGIN_MESSAGE_MAP(CSchKillFileAppDlg, CDialog)
	...
	ON_REGISTERED_MESSAGE(UWM_ARE_YOU_ACTIVE, OnAreYouActive)
	ON_WM_DROPFILES()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDOWN()
	ON_WM_CLOSE()
...
LRESULT CSchKillFileAppDlg::OnAreYouActive(WPARAM wParam, LPARAM lParam)
{
	// Returns a non-zero value to inform the searcher: "This is the instance you are looking for."
	return 1;
}
...
void CSchKillFileAppDlg::OnBnClickedForceDelete()
{
	...
	for (int j = 0; j < m_list.GetItemCount(); j++)
	{
		...
		WCHAR szwPath[512] = { 0 };
		MultiByteToWideChar(CP_ACP, 0, str, -1, szwPath, _countof(szwPath));

		DWORD dwRet = 0;
		DWORD ctrl = isDir ? IOCTL_DELETEFOLDER : IOCTL_DELETEFILE;

		// Execute IOCTL
		BOOL bSuccess = DeviceIoControl(hDev, ctrl, szwPath, sizeof(szwPath), NULL, 0, &dwRet, NULL);
		...
		if (!bSuccess)
		{
			...
			{
				CString msg;
				msg.Format("The file or directory is locked by the kernel and cannot be deleted immediately (Error code: %lu).\nPath: %s\n\nWould you like to safely remove it on the next system reboot?", dwError, str);

				if (MessageBox(msg, "Delay Delete Confirmation", MB_YESNO | MB_ICONQUESTION) == IDYES)
				{
					MarkForDeleteOnReboot(str);
					MessageBox("Scheduled successfully! It will be deleted on next boot.", "Info", MB_ICONINFORMATION);

					// Remove from list since it's scheduled
					m_list.DeleteItem(j);
					j--;
				}
			}
			...
			{
				CString err;
				err.Format("Deletion failed. Error code: %lu", dwError);
				MessageBox(err, "Error", MB_ICONERROR);
			}
		}
		else
		{
			// Immediate deletion was successful
			m_list.DeleteItem(j);
			j--;
		}
	}
...
{
	// 1. Create the FileOpenDialog object
	CComPtr<IFileOpenDialog> pFileOpen;
	HRESULT hr = pFileOpen.CoCreateInstance(CLSID_FileOpenDialog);

	if (FAILED(hr))
	{
		// Fallback for extremely old systems or COM failure
		return;
	}

	// 2. Set dialog options
	DWORD dwFlags;
	hr = pFileOpen->GetOptions(&dwFlags);
	if (SUCCEEDED(hr))
	{
		// FOS_NOVALIDATE: Crucial. This skips the "file in use" or "access denied" validation
		// FOS_FORCEFILESYSTEM: Ensures we get actual file system paths
		pFileOpen->SetOptions(dwFlags | FOS_ALLOWMULTISELECT | FOS_NOVALIDATE | FOS_PATHMUSTEXIST | FOS_FORCEFILESYSTEM);
	}
	...
	if (SUCCEEDED(hr))
	{
		// 5. Get the results from the dialog
		...
		if (SUCCEEDED(hr))
		{
			...
			for (DWORD i = 0; i < dwCount; ++i)
			{
				...
				if (SUCCEEDED(hr) && pszFilePath != NULL)
				{
					...
					// Manually free the string allocated by Shell
					CoTaskMemFree(pszFilePath);

					// Extract file name for the list display
					int nPos = strPath.ReverseFind('\\');
					CString strName = (nPos != -1) ? strPath.Mid(nPos + 1) : strPath;

					// Update the UI list control
